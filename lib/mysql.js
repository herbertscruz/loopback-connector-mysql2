...
var debug = require('debug')('loopback:connector:mysql2');
var setHttpCode = require('./set-http-code');

/**
 * @module loopback-connector-mysql2
 *
...
function MySQL(settings) {
  SqlConnector.call(this, 'mysql2', settings);
}
...
MySQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  function executeWithConnection(err, connection) {
    ...
    if (self.settings.createDatabase) {
      // Call USE db ...
      connection.execute('USE ??', [db], function(err) {
        if (err) {
          if (err && err.message.match(/(^|: )unknown database/i)) {
            ...
            connection.execute(q, [db, charset, collation], function(err) {
              if (!err) {
                connection.execute('USE ??', [db], function(err) {
                  runQuery(connection);
                });
